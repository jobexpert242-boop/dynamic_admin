...
const taxTotal = computed(() =>
    form.items.reduce((sum, item) => sum + (Number(item.qty) * Number(item.price) * Number(item.tax || 0)) / 100, 0)
);

const discountTotal = computed(() => {
    if (!form.discount || !form.discount_type) return 0;
    if (form.discount_type === "percent") return (subTotal.value * Number(form.discount)) / 100;
...
function openDiscount() { showDiscountModal.value = true; }
function applyDiscount() { showDiscountModal.value = false; }

// Customer options
const customerOptions = props.users.map(u => ({ value: u.id, label: u.name }));

// Auto-fill address on customer select
...
<template>
    <Layout>
        ...
        <FlashMessage v-if="$page.props.flash?.status" :message="$page.props.flash.status" type="success" />

        <div class="border border-gray-300 p-2 shadow-sm rounded">
            <div class="flex justify-between items-center border-b mb-4 pb-3">
                <h2 class="text-lg font-bold"><i class="fa fa-pencil"></i> Edit Billing</h2>
                <Link :href="route('admin.billing')" class="btn rounded-sm"><i class="fa-solid fa-list"></i> Invoice List</Link>
            </div>
                ...
                <div class="flex gap-3">
                    ...
                    <div class="w-3/4 h-fit">
                        <div class="border border-gray-300 bg-white p-3 mb-3">
                            ...
                            <div v-for="(item, index) in form.items" :key="index">
                                <div class="flex gap-4 mb-2 items-center">
                                    ...
                                    <div>
                                        ...
                                        <input type="date" v-model="item.warranty_date" class="border rounded p-2 border-gray-400"/>
                                    </div>
                                    <TextInput
                                        label="IMEI/SL"
                                        v-model="item.imei_sl"
                                        :error="form.errors[`items.${index}.imei_sl`]"
                                    />
                                    <div>
                                        <label class="form-label">Note</label>
                                        <textarea v-model="item.note" class="border rounded p-2 border-gray-400"></textarea>
                                    </div>
                                    <TextInput
                                        label="Qty"
                                        type="number"
                                    ...
                                    <div>
                                        ...
                                        <select v-model="item.tax" class="border rounded p-2 border-gray-400">
                            ...
                            <button type="button" @click="addItem" class="btn rounded-sm">+ Add Item</button>
                        </div>
                    ...
                    <div class="w-1/4 h-fit">
                        <div class="flex gap-5 justify-end p-3 border border-gray-300 bg-white">
                            <button type="submit" class="btn rounded-sm flex justify-center items-center gap-3" :disabled="form.processing" :class="{ 'btn-spinner': form.processing }">
                        ...
                        <!-- TOTALS -->
                        <div class="p-3 border border-gray-300 bg-white mt-2 flex justify-between">
                            <label class="form-label cursor-pointer">
                                <input type="checkbox" v-model="roundOff" /> Round Off
                            </label>
                            <div class="text-end">
                                <p class="text-xl">Sub Total: {{ subTotal.toFixed(2) }}</p>
                                <p class="text-xl">Discount: {{ discountTotal.toFixed(2) }}</p>
                                <p class="text-xl">Tax: {{ taxTotal.toFixed(2) }}</p>
                                <p class="font-bold text-2xl">TOTAL: {{ grandTotal.toFixed(2) }}</p>
                            </div>
                        </div>
                        ...
                        <div class="border border-gray-300 bg-white p-3 mt-3">
                            ...
                            <div class="mb-3">
                                <TextInput label="Paid Amount" v-model="form.paid" :error="form.errors.paid"/>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Invoice Date</label>
                                <input type="date" v-model="form.invoice_date" class="border rounded p-2 border-gray-300 w-full"/>
                                <small class="text-red-500" v-if="form.errors.invoice_date">{{ form.errors.invoice_date }}</small>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Status</label>
                                <select v-model="form.status" class="border rounded p-2 border-gray-300 w-full">
                            ...
                            <button type="button" class="btn rounded-sm mt-3" @click="openDiscount"><i class="fa fa-plus"></i> Add Discount</button>
                        </div>
                    </div>
                </div>
            </form>
        ...
        <!-- Discount modal -->
        <div v-if="showDiscountModal" class="fixed inset-0 bg-black/50 flex justify-center items-center">
            <div class="bg-white p-5 rounded shadow-lg w-96">
                <h2 class="text-lg font-bold mb-3">Apply Discount</h2>
                <div class="mb-3">
                    <label class="form-label">Discount Type</label>
                    <select v-model="form.discount_type" class="border rounded p-2 border-gray-400 w-full">
                        <option value="">Select Type</option>
                        <option value="percent">Percent (%)</option>
                        <option value="fixed">Fixed Amount</option>
                    </select>
                </div>
                <TextInput label="Discount Amount" v-model="form.discount" type="number"/>
                <div class="flex justify-end gap-3 mt-4">
                    <button class="btn rounded-sm" @click="showDiscountModal=false">Cancel</button>
